...
$result = mysqli_query($open, 'SELECT DAY(`date`) as `day`, MONTHNAME(`date`) as `monthname`, DATE_FORMAT(`date`,"%H:%i") as `date_formated`, `order_id`, `net_price`, `status` FROM `cp_orders` WHERE `date` BETWEEN DATE_SUB(NOW(), INTERVAL 30 DAY) AND NOW() ORDER BY `date` DESC');
if ($result == FALSE) {
